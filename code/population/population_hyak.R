...
for(lag in lags){
  ...
  tfr.dir <- paste0(work.path, "TFR/TFR_lag_", toString(lag))
  if(!dir.exists(tfr.dir)){
    dir.create(tfr.dir, recursive = TRUE)
  }
  
  e0.dir <- paste0(work.path, "E0/E0_lag_", toString(lag))
  if(!dir.exists(e0.dir)){
    dir.create(e0.dir, recursive = TRUE)
  }
  
  mig.dir <- paste0(work.path, "MIG/MIG_lag_", toString(lag))
  if(!dir.exists(mig.dir)){
    dir.create(mig.dir, recursive = TRUE)
  }
  
  pop.dir <- paste0(work.path, "POP/POP_lag_", toString(lag))
  ...
  if (lag==0){
    tfr.m <- tfr.shift.estimation.to.wpp(tfr.dir, burnin = tfr.burnin)
  }
  ...
  if (lag==0){
    e0.shift.prediction.to.wpp(e0.dir, stat = "mean") # female
    e0.shift.prediction.to.wpp(e0.dir, stat = "mean", joint.male = TRUE) # male
  }
